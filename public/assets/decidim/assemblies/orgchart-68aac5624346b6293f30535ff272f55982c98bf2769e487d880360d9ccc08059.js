...
(function (exports) {
  var render = exports.Decidim.Visualizations;
  ...
  var renderOrgCharts = function renderOrgCharts() {
    ...
    var renderChartCollapsibleNetwork = function renderChartCollapsibleNetwork(params) {
      ...
      var main = function main(selection) {
        selection.each(function scope() {
          ...
          // add svg
          var svg = container.patternify({ tag: "svg", selector: "svg-chart-container" }).attr("width", attrs.svgWidth).attr("height", attrs.svgHeight);
          // .call(behaviors.zoom)

          // add container g element
          var chart = svg.patternify({ tag: "g", selector: "chart" }).attr("transform", "translate(" + calc.chartLeftMargin + "," + calc.chartTopMargin + ")");
          ...
          function ticked() {
            ...
            svg.selectAll(".node").attr("transform", function (d) {
              return "translate(" + maxXValueAvailable(d.x) + "," + maxYValueAvailable(d.y) + ")";
            });
          }
          ...
          function nodeMouseEnter(d) {
            // get links
            var _links = hierarchy.root.links();
      ...
      d3.selection.prototype.patternify = function (_params) {
        ...
        // pattern in action
        var selection = this.selectAll("." + selector).data(_data);
    ...
    $orgChartContainer.each(function (i, container) {
      ...
      d3.json($container.data("url")).then(function (data) {
        ...
        orgchart = renderChartCollapsibleNetwork().svgHeight(height).svgWidth(width).fakeRoot(fake).container("#" + container.id).data({
          root: dataDepicted
        }).debug(true).run();
      });
    });
